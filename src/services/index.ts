...
import { TencentCredentials, rawDataInterface } from '../types'
import {
  DEFAULT_REGION,
  DEFAULT_RESOURCES,
...
export default class Provider extends CloudGraph.Client {
  ...
  async askForTencentCredentials(): Promise<TencentCredentials> {
    return this.interface.prompt([
      {
        type: 'input',
        message: 'Please input a valid secret ID: ',
        name: 'secretId',
      },
      {
        type: 'input',
        message: 'Please input a valid secret key: ',
        name: 'secretKey',
      },
    ])
  }
  ...
  private printTencentCredentials(account: TencentCredentials): void {
    this.logger.success(
      `secretId: ${chalk.underline.green(account.secretId)}`
    )
  }
  ...
  async configure(): Promise<{ [key: string]: any }> {
    ...
    const accounts: TencentCredentials[] = []
    ...
    while (true) {
      if (accounts.length > 0) {
        const { addSubscription } = await this.interface.prompt([
          {
            type: 'confirm',
            message: 'Configure another Tencent account?',
      ...
      const { secretId, secretKey }: TencentCredentials =
        await this.askForTencentCredentials()
    ...
    if (!accounts.length) {
      accounts.push({ secretId: '', secretKey: '' })
    }
  ...
  private async getRawData(
    ...
    const { secretId } = account
    const config = { ...account }

    this.printTencentCredentials(account)
    ...
    try {
      ...
      this.logger.success(`${secretId} scan completed`)
    } catch (error: any) {
  ...
  async getData({ opts }: { opts: Opts }): Promise<ProviderData> {
    ...
    let { regions: configuredRegions, resources: configuredResources } =
      this.config
    const { accounts: configuredAccounts }: { accounts: TencentCredentials[] } =
      this.config
    if (!configuredRegions) {
      configuredRegions = this.properties.regions.join(',')
    } else {
    ...
    for (const account of configuredAccounts) {
      ...
      if (!crawledAccounts.find(val => val === secretId)) {
        crawledAccounts.push(secretId)
        ...
        this.logger.warn(
          ...
          `${secretId} has already been crawled, skipping...`
        )
      }
    }
